...
def generate_zcr_output(data, frame_length):
    data_dims = data.shape[0]
    if frame_length != 1:
        output_data = np.zeros((data_dims, len(data[0]) * frame_length))
        for dim in range(data_dims):  # Looping through no. of channels (expect 1 for mono)
...
def generate_spectral_centroid_output(data, frame_length):
    data_dims = data.shape[0]
    if frame_length != 1:
        output_data = np.zeros((data_dims, len(data[0]) * frame_length))
        for dim in range(data_dims):  # Looping through no. of channels (expect 1 for mono)
...
def write_wav_file(file_name, data, feature, sample_rate=48000, frame_length=2048, input_stream=None):
    if feature == 'rms':
        ...
        output_data = generate_zcr_output(data, frame_length)
    elif feature == 'spectral_centroid':
        output_data = generate_spectral_centroid_output(data, frame_length)
